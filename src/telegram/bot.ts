...
export const parseSessionSelection = (data: string): string => data.replace("session:select:", "");
...
export const parseApprovalAction = (data: string): { sessionId: string; key: ApprovalActionKey | "" } => {
...
export const parseApprovalTokenAction = (
  ...
  const { sessionId: requestToken, suffix } = parseTrailingSegment(data, "approvalToken:");
  const key = (
    ["Enter", "y", "p", "Escape", "n", "C-c"].includes(suffix)
      ? suffix
      : ""
...
export const parseToolOutputAction = (
  ...
  const { sessionId: requestToken, suffix } = parseTrailingSegment(data, "toolOutput:");
  return {
    requestToken,
...
const buildSessionControlKeyboard = (sessionId: string): InlineKeyboard | undefined => {
  const callbackData = [
...
export const getGroupReadinessAdvice = ({
  ...
  if (enableForumTopics && isForum !== "true") {
    return {
      recommendedState: "建议目标状态：bot 为 administrator，canManageTopics=true，群已开启 Topics。",
    ...
    .map((line, index) => {
      ...
      if (sectionLabel) {
        ...
        const sectionBody = new RegExp(`^${escapeRegExp(sectionLabel)}[：:]?$`).test(trimmed)
          ? ""
          : trimmed;
        const renderedSectionBody = renderSemanticParagraph(sectionBody, semanticAssistant);
        return sectionBody
